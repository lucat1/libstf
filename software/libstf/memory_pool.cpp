...
std::string mallctl_error_to_string(int error) {
  switch (error) {
  case EINVAL:
    return "The alignment parameter is not a power of 2 at least as large as "
           "sizeof(void "
           "*).";
  case ENOENT:
    return "Name or mib specifies an unknown/invalid value.";
  case EPERM:
    return "Attempt to read or write void value, or attempt to write read-only "
           "value.";
  case EAGAIN:
...
void check_mallctl_success(int error, std::string msg) {
  if (error != 0) {
    throw std::runtime_error(msg +
                             " Got error: " + mallctl_error_to_string(error));
  }
}
...
void je_mallctl_do(std::string control_name) {
  check_mallctl_success(
      je_mallctl(control_name.c_str(), nullptr, nullptr, nullptr, 0),
...
void je_mallctl_write(std::string control_name, A *value) {
  check_mallctl_success(
      je_mallctl(control_name.c_str(), nullptr, nullptr, value, sizeof(A)),
...
template <typename A> A je_mallctl_read(std::string control_name) {
  ...
  auto size = sizeof(A);
  check_mallctl_success(
      je_mallctl(control_name.c_str(), &value, &size, nullptr, 0),
...
A je_mallctl_read_write(std::string control_name, B *write_value) {
  A read_value;
  auto size = sizeof(A);
  check_mallctl_success(je_mallctl(control_name.c_str(), &read_value, &size,
                                   write_value, sizeof(B)),
                        "Failed to read/write from mallctl control " +
                            control_name + ".");
...
alignas(HugePageMemoryPool::DEFAULT_ALIGNMENT) int64_t zero_size_data[1] = {
    0xFFFFULL};
...
HugePageMemoryPool::HugePageMemoryPool() {
  ...
  next_free_addr =
      mmap(nullptr, num_huge_pages * PAGE_SIZE, PROT_READ | PROT_WRITE,
           MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | HUGE_PAGE_TYPE, -1, 0);
  if (next_free_addr == MAP_FAILED) {
    throw std::runtime_error("Could not allocated the expected number of 1GiB "
                             "huge pages for HugePageMemoryPool.");
  }
  ...
  arena_index = je_mallctl_read_write<unsigned, extent_hooks_t *>(
      "arenas.create", &hooks);
}

HugePageMemoryPool::~HugePageMemoryPool() {
  // Destroy all the tcaches we crated
  // This is REQUIRED for destroying the arena and otherwise will lead to a
  // SEGFAULT
  ...
  // we sleep a fixed time here in the hope that after this, the tcache are
  // destroyed.
  ...
  if (munmap(initial_address_, total_capacity_) == -1) {
    std::cerr << "HugePageMemoryPool: Could not munmap the obtained huge page "
                 "mappings."
              << std::endl;
  }
}
...
int HugePageMemoryPool::get_number_of_available_huge_pages() {
  ...
  path << "/sys/kernel/mm/hugepages/hugepages-" << PAGE_SIZE_KB
       << "kB/free_hugepages";
  std::ifstream file(path.str());

  if (!file.is_open()) {
    throw std::runtime_error(
        "It seems the target system does not have 1GiB huge pages enabled, "
        "which are required "
  ...
  file.close();
  if (free_pages == 0) {
    throw std::runtime_error("Your system has 0 free 1GiB huge pages. The FPGA "
                             "support requires 1GiB huge pages. "
                             "Please enable additional 1GiB huge pages as "
                             "described in the Maximus readme.");
  }
  return free_pages;
}
...
unsigned HugePageMemoryPool::get_tcache_id_for_calling_thread() {
  ...
  // The Problem is that when we want to destroy the arena, jemalloc requires us
  // to first, destroy all the (automatically created) tcaches. Since they are
  // automatically managed, and we don't know how many threads called our memory
...
bool HugePageMemoryPool::is_in_bounds(void *ptr, size_t size) {
  return is_buffer_within_bounds({initial_address_, total_capacity_},
                                 {ptr, size});
}

std::pair<void *, size_t>
HugePageMemoryPool::get_page_boundaries(const void *ptr) {
  ...
  auto initial =
      reinterpret_cast<std::byte *>(const_cast<void *>(initial_address_));
  auto buf = reinterpret_cast<std::byte *>(const_cast<void *>(ptr));
  auto n_th_page = (buf - initial) / PAGE_SIZE;
  return std::make_pair(static_cast<void *>(initial + n_th_page * PAGE_SIZE),
                        PAGE_SIZE);
}
...
Status HugePageMemoryPool::allocate(size_t size, size_t alignment, void **out) {
  if (size == 0) {
    *out = ZeroSizePointer;
    ...
    // Ensure the alignment is a power of two. See:
    // https://stackoverflow.com/a/108360/5589776
    assert((alignment & (alignment - 1)) == 0);
    auto tc = get_tcache_id_for_calling_thread();
    *out =
        je_mallocx(size, MALLOCX_ALIGN(alignment) | MALLOCX_ARENA(arena_index) |
                             MALLOCX_TCACHE(tc));
    if (*out == nullptr) {
...
Status HugePageMemoryPool::reallocate(size_t old_size, size_t new_size,
  ...
  *ptr = je_rallocx(*ptr, new_size,
                    MALLOCX_ALIGN(alignment) | MALLOCX_ARENA(arena_index) |
                        MALLOCX_TCACHE(tc));
...
void *HugePageMemoryPool::huge_page_alloc(extent_hooks_t *hooks, void *new_addr,
  ...
  auto aligned_address =
      std::align(alignment, size, next_free_addr,
                 // Note: std::align decreases remaining_capacity automatically
                 // by the alignment bytes!
                 remaining_capacity);

  // There was not enough space remaining
  if (aligned_address == nullptr) {
    allocations_mutex_.unlock();
    std::cerr << "HugePageMemoryPool: Not enough huge page memory remaining to "
                 "satisfy "
                 "jemalloc request over "
              << size
              << " bytes. Please add additional 1GiB huge pages to your system."
              << std::endl;
    return nullptr;
  }
  ...
  next_free_addr =
      static_cast<void *>(static_cast<std::byte *>(aligned_address) + size);

  // MAP_ANONYMOUS always ensures zero-ing of the memory and only returned
  // commit memory
  *zero = true;
  *commit = true;
  allocations_mutex_.unlock();
...
bool HugePageMemoryPool::huge_page_dealloc(extent_hooks_t *hooks, void *addr,
...
bool HugePageMemoryPool::huge_page_decommit(extent_hooks_t *hooks, void *addr,
...
bool HugePageMemoryPool::huge_page_split_extend(extent_hooks_t *hooks,
  ...
  // -> We don't really need to do anything, everything remains in one big
  // chunk.
  return false;
}
...
bool HugePageMemoryPool::huge_page_merge_extend(extent_hooks_t *hooks,
  ...
  // Since all extends given to jemalloc are already continuous, we can always
  // return false
  return false;
}
...
Status SimpleMemoryPool::reallocate(size_t old_size, size_t new_size,
                                    size_t alignment, void **ptr) {
  if (new_size == 0) {
    free(*ptr, old_size, alignment);
    *ptr = nullptr;
...
std::pair<void *, size_t>
SimpleMemoryPool::get_page_boundaries(const void *ptr) {
  std::lock_guard<std::recursive_mutex> lock(allocated_buffers_mutex);

  for (const auto &buffer : allocated_buffers) {
    if (is_buffer_within_bounds({buffer.first, buffer.second},
                                {const_cast<void *>(ptr), 0})) {
  ...
  std::ostringstream err;
  err << "The Provided address " << ptr
      << " is not within the bounds of the memory pool!";
  throw std::runtime_error(err.str());
}
