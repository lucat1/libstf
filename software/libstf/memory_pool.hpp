...
public:
   ...
   * @return Status indicating success or failure.
   */
  virtual Status reallocate(size_t old_size, size_t new_size, size_t alignment,
                            void **ptr) = 0;
  ...
  /**
   * Returns the address and size for the page in which the given allocation was
   * placed. This information can be used, e.g. for TLB mappings on FPGAs.
   * @param ptr The address where the allocation begins, as returned by
   * 'allocate'
...
// A static piece of memory for 0-size allocations, to return an aligned
// non-null pointer. This is required because Arrow memory pools (when we use
// this in other projects) need to support 0-byte allocations, reallocations,
// and deallocations but jemalloc does not support them!
 ...
 * This is required for the FPGA support since all the data send/received from
 * the FPGA needs to be mapped on the FPGAs TLB. Additionally, every TLB miss
 * causes an FPGA-side interrupt and is handled in Coyotes kernel code. As small
 * pages lead to many TLB misses, this can cause performance problems due to the
 * large number of interrupts. The goal of this pool is to minimize such misses
 * by using huge pages.
 *
 * The pool is implemented as follows: During initialization, it pre-allocates
 * all available 1GiB pages in the system.  Under the hood it uses jemalloc to
 * handle the actual allocation/free requests. Jemalloc uses smart (and very
 * complex) mechanisms to minimize fragmentation. However, whenever it runs out
 * of memory, it asks this pool via extension_hooks, which provides a chunk of
 * the pre-allocated memory. The pre-allocated 1GiB pages will be unmaped/freed
 * when the MemoryPool is destroyed.
 */
...
public:
  ...
  static inline const size_t HUGE_PAGE_TYPE =
      (HUGE_PAGE_BITS << MAP_HUGE_SHIFT);
  ...
  Status reallocate(size_t old_size, size_t new_size, size_t alignment,
                    void **ptr) override;

  void free(void *ptr, size_t size, size_t alignment) override;
  void free(void *ptr, size_t size) override {
    free(ptr, size, DEFAULT_ALIGNMENT);
  }
   ...
   * @return Whether the given buffer and size have been allocated by this
   * memory pool
   */
...
private:
  /**
   * @return The number of huge pages with PAGE_SIZE currently available in the
   * system. Ensures this number is > 0
   */
  int get_number_of_available_huge_pages();

  // Call back functions that get called by jemalloc to manage the underlying
  // memory
  static void *huge_page_alloc(extent_hooks_t *hooks, void *new_addr,
                               size_t size, size_t alignment, bool *zero,
                               bool *commit, unsigned arena_ind);

  static bool huge_page_dealloc(extent_hooks_t *hooks, void *addr, size_t size,
                                bool committed, unsigned arena_ind);

  static bool huge_page_decommit(extent_hooks_t *hooks, void *addr, size_t size,
                                 size_t offset, size_t length,
                                 unsigned arena_ind);

  static bool huge_page_split_extend(extent_hooks_t *hooks, void *addr,
                                     size_t size, size_t size_a, size_t size_b,
                                     bool committed, unsigned arena_ind);

  static bool huge_page_merge_extend(extent_hooks_t *hooks, void *addr_a,
...
/**
 * Implements a naive memory pool that is only used for simulation purposes in
 * systems where there are no huge pages.
 */
...
private:
  ...
  // Recursive mutex to protect allocated_buffers (recursive to allow destructor
  // to call free())
